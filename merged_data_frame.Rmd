...
state_avgPR <- merged_data %>%
  group_by(STATE) %>%
  summarize(avgPR = mean(log(BARTHAG), na.rm = TRUE))

geo <- geojson_read("states.geo.json", what = "sp")

geo@data <- left_join(geo@data, state_avgPR, by = c("STATE" = "STATE"))
...
leaflet(geo) %>%
  addPolygons(fillColor = ~pal(state_avgPR$BARTHAG))


#(title = "Average Power Ratings of NCAA D1 Basketball Teams by State")
